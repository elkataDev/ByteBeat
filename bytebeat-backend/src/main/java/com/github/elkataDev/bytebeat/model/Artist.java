...
public class Artist {
    ...
    @Column(name = "total_followers", nullable = false) // contador seguidores
    private int totalFollowers = 0; // Inicializado a 0

    // Relacion inversa para saber qué usuarios siguen a este artista
    @ManyToMany(mappedBy = "followedArtists")
    private List<User> followers = new ArrayList<>();

    // OPCIONAL: Sirve para que puedas instanciar objetos nuevos rápidamente antes
    // de meterlos en la base de datos por ejemplo un POST
    // En este caso crear artistas nuevos a mano (ej. en un POST) sin escribir IDs
    // ya que son incrementados
    ...
    public Artist(String name, String biography, String profilePictureUrl) {
        ...
        this.totalFollowers = 0; // Inicializado a 0
